    ...
    **must** call ``super()._acting(...)`` / ``super()._reasoning(...)``
    ...
    def _setup_memory_manager(
        ...
        if self._enable_memory_manager and self.memory_manager is not None:
            ...
            # Register memory_add as a tool function (for MemOS Cloud)
            self.toolkit.register_tool_function(
                create_memory_add_tool(self.memory_manager),
                namesake_strategy=namesake_strategy,
            )
            logger.debug("Registered memory_add tool")

            # Register memory_feedback tool (for MemOS Cloud feedback API)
            self.toolkit.register_tool_function(
                create_memory_feedback_tool(self.memory_manager),
                namesake_strategy=namesake_strategy,
            )
            logger.debug("Registered memory_feedback tool")

            # Register memory_get tool (for MemOS Cloud)
            self.toolkit.register_tool_function(
                create_memory_get_tool(self.memory_manager),
                namesake_strategy=namesake_strategy,
            )
            logger.debug("Registered memory_get tool")

            # Register memory_delete tool (for MemOS Cloud)
            self.toolkit.register_tool_function(
                create_memory_delete_tool(self.memory_manager),
                namesake_strategy=namesake_strategy,
            )
            logger.debug("Registered memory_delete tool")

            # Register task_status tool (for async task monitoring)
            self.toolkit.register_tool_function(
                create_task_status_tool(self.memory_manager),
                namesake_strategy=namesake_strategy,
            )
            logger.debug("Registered task_status tool")

            # Register knowledgebase tools
            kb_tools = create_knowledgebase_tools(self.memory_manager)
            for tool_name, tool_func in kb_tools.items():
                self.toolkit.register_tool_function(
                    tool_func,
                    namesake_strategy=namesake_strategy,
                )
                logger.debug(f"Registered {tool_name} tool")
    ...
    def _register_hooks(self) -> None:
        ...
        # Memory compaction hook - DISABLED (using MemOS for memory management)
        # if self._enable_memory_manager and self.memory_manager is not None:
        ...
        # MemOS recall hook - auto-recall memories before reasoning
        if self._enable_memory_manager and self.memory_manager is not None and self.memory_manager.memos_enabled:
            from .hooks import MemosRecallHook, MemosAddHook
            import json
            from pathlib import Path
            ...
            # Get recall config
            recall_config = memos_config.get("recall", {})
            filter_config = recall_config.get("filter", {})
            conversation_config = memos_config.get("conversation", {})
            ...
            memos_recall_hook = MemosRecallHook(
                ...
                knowledgebase_ids=recall_config.get("knowledgebaseIds", []),
                tags=recall_config.get("tags", []),
                # Recall filter
                recall_filter_enabled=filter_config.get("enabled", False),
                recall_filter_base_url=filter_config.get("baseUrl", "http://127.0.0.1:11434/v1"),
                recall_filter_api_key=filter_config.get("apiKey", ""),
                recall_filter_model=filter_config.get("model", "qwen2.5:7b"),
                recall_filter_timeout_ms=filter_config.get("timeoutMs", 6000),
                recall_filter_retries=filter_config.get("retries", 0),
                recall_filter_candidate_limit=filter_config.get("candidateLimit", 30),
                recall_filter_max_item_chars=filter_config.get("maxItemChars", 500),
                recall_filter_fail_open=filter_config.get("failOpen", True),
                # Phase 3: Conversation management
                conversation_id_prefix=conversation_config.get("idPrefix", ""),
                conversation_id_suffix=conversation_config.get("idSuffix", ""),
                conversation_suffix_mode=conversation_config.get("suffixMode", "none"),
                reset_on_new=conversation_config.get("resetOnNew", True),
            )
            self.register_instance_hook(
                hook_type="pre_reasoning",
                hook_name="memos_recall_hook",
                hook=memos_recall_hook.__call__,
            )
            logger.info("Registered MemOS recall hook")

            # Get add config
            add_config = memos_config.get("add", {})

            memos_add_hook = MemosAddHook(
                memory_manager=self.memory_manager,
                capture_strategy=add_config.get("captureStrategy", "last_turn"),
                include_assistant=add_config.get("includeAssistant", False),
                throttle_ms=add_config.get("throttleMs", 0),
                max_message_chars=add_config.get("maxMessageChars", 2000),
                tags=add_config.get("tags", []),
                async_mode=add_config.get("asyncMode", True),
                multi_agent_mode=add_config.get("multiAgentMode", False),
            )
            self.register_instance_hook(
                hook_type="post_reply",
                hook_name="memos_add_hook",
                hook=memos_add_hook.__call__,
            )
            logger.info("Registered MemOS add hook")
